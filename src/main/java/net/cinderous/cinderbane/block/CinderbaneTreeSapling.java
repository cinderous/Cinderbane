...
public class CinderbaneTreeSapling extends BushBlock implements IGrowable {
    public static final IntegerProperty STAGE = BlockStateProperties.STAGE_0_1;
    protected static final VoxelShape SHAPE = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 12.0D, 14.0D);
    private final Tree tree;

    public CinderbaneTreeSapling(Tree treeIn, AbstractBlock.Properties properties) {
        super(properties);
        this.tree = treeIn;
    ...
    protected boolean isValidGround(BlockState state, IBlockReader worldIn, BlockPos pos) {
        return state.isIn(Blocks.GRASS_BLOCK) || state.isIn(Blocks.DIRT) || state.isIn(Blocks.COARSE_DIRT) || state.isIn(Blocks.PODZOL) || state.isIn(Blocks.FARMLAND) || state.isIn(CinderbaneRegistry.CINDIRT.get()) || state.isIn(CinderbaneRegistry.CINDIRT_GRASS_BLOCK.get());
    }
}